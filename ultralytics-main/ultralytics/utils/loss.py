...
class BboxLoss(nn.Module):
    ...
    def __init__(
        ...
        self.shape_iou_lambda = float(shape_iou_lambda)

    def _shape_iou(self, pred_bboxes: torch.Tensor, target_bboxes: torch.Tensor, eps: float = 1e-7) -> torch.Tensor:
        """ShapeIoU-like quality (higher is better), xyxy input."""
        ...
        ww = 2 * torch.pow(w2, self.shape_iou_scale) / (
            torch.pow(w2, self.shape_iou_scale) + torch.pow(h2, self.shape_iou_scale) + eps
        )
        hh = 2 * torch.pow(h2, self.shape_iou_scale) / (
            torch.pow(w2, self.shape_iou_scale) + torch.pow(h2, self.shape_iou_scale) + eps
        )
        ...
        c2 = cw**2 + ch**2 + eps
        cdx = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2) / 4
        cdy = ((b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4
        distance = (hh * cdx + ww * cdy) / c2
        ...
        return iou - distance - 0.5 * self.shape_iou_lambda * shape_cost
    ...
    def forward(
        ...
        if self.use_shape_iou:
            iou = self._shape_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask])
        else:
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        self.use_shape_iou = bool(getattr(h, "shape_iou", False))
        self.shape_iou_scale = float(getattr(h, "shape_iou_scale", 0.0))
        self.shape_iou_lambda = float(getattr(h, "shape_iou_lambda", 1.3))
        self.use_ema_slide_cls = bool(getattr(h, "ema_slide_cls", False))
        self.ema_slide_decay = float(getattr(h, "ema_slide_decay", 0.999))
        self.ema_slide_tau = float(getattr(h, "ema_slide_tau", 2000.0))
        self._slide_updates = 0
        self._slide_iou_mean = 1.0
        self.use_apt_tal = bool(getattr(h, "apt_tal", False))
        self.apt_tal_power = float(getattr(h, "apt_tal_power", 2.0))
        self.apt_tal_quality_gamma = float(getattr(h, "apt_tal_quality_gamma", 1.0))
        ...
        assigner_cls = APTTaskAlignedAssigner if self.use_apt_tal else TaskAlignedAssigner
        self.assigner = assigner_cls(
            ...
            topk2=tal_topk2,
            **(
                {"power": self.apt_tal_power, "quality_gamma": self.apt_tal_quality_gamma}
                if self.use_apt_tal
                else {}
            ),
        )
        self.bbox_loss = BboxLoss(
            m.reg_max,
            use_shape_iou=self.use_shape_iou,
            shape_iou_scale=self.shape_iou_scale,
            shape_iou_lambda=self.shape_iou_lambda,
        ).to(device)
        self.proj = torch.arange(m.reg_max, dtype=torch.float, device=device)

    def _ema_decay(self, x: int) -> float:
        return self.ema_slide_decay * (1 - math.exp(-x / max(self.ema_slide_tau, 1.0)))

    def _update_slide_iou(self, auto_iou: float) -> None:
        self._slide_updates += 1
        d = self._ema_decay(self._slide_updates)
        self._slide_iou_mean = d * self._slide_iou_mean + (1 - d) * float(auto_iou)

    def _ema_slide_weights(self, true: torch.Tensor) -> torch.Tensor:
        auto_iou = max(float(self._slide_iou_mean), 0.2)
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        if self.use_ema_slide_cls:
            auto_iou = 0.5
            if fg_mask.sum():
                iou_fg = bbox_iou(
                    (pred_bboxes.detach() * stride_tensor)[fg_mask],
                    target_bboxes[fg_mask],
                    xywh=False,
                    CIoU=True,
                ).clamp(0, 1)
                auto_iou = float(iou_fg.mean().detach()) if iou_fg.numel() else auto_iou
            self._update_slide_iou(auto_iou)
            bce_loss *= self._ema_slide_weights(target_scores.to(dtype))
