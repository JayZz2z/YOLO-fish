...
class APTTaskAlignedAssigner(TaskAlignedAssigner):
    """APT-TAL variant of TaskAlignedAssigner.

    Two lightweight changes (training-only):
      1) overlap power-transform before alignment metric
      2) optional quality-gamma on normalized alignment score
    """

    def __init__(self, *args, power: float = 2.0, quality_gamma: float = 1.0, **kwargs):
        super().__init__(*args, **kwargs)
        self.power = float(power)
        self.quality_gamma = float(quality_gamma)

    @staticmethod
    def power_transform(array: torch.Tensor, power: float = 2.0) -> torch.Tensor:
        return torch.where(array < 0.5, array.pow(power), array.pow(1.0 / max(power, 1e-6)))

    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        align_metric, overlaps = super().get_box_metrics(pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt)
        if self.power != 1.0:
            overlaps = self.power_transform(overlaps.to(torch.float), power=self.power).to(align_metric.dtype)
            # recompute align metric using transformed overlap
            na = pd_bboxes.shape[-2]
    ...
    def _forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        norm_align_metric = (align_metric * pos_overlaps / (pos_align_metrics + self.eps)).amax(-2).unsqueeze(-1)
        if self.quality_gamma != 1.0:
            norm_align_metric = norm_align_metric.clamp(min=0).pow(self.quality_gamma)
        target_scores = target_scores * norm_align_metric
