    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in repeat_modules:
                ...
                n = 1
            if m is C3k2 or m is RepC3k2:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is A2C2f:
        ...
        elif m is DualGateConcat:
            # 2-input bilateral-reweighted concat; c_up/c_lat auto-filled from graph.
            # YAML args support: [] or [gamma_init] or [gamma_init, hidden].
            assert isinstance(f, (list, tuple)) and len(f) == 2, (
                f"DualGateConcat expects exactly 2 inputs, got {f}"
            )
            c_up, c_lat = ch[f[0]], ch[f[1]]
            extra = list(args) if args else []
            args = [c_up, c_lat, *extra]
            c2 = c_up + c_lat
        elif m in frozenset(
            {
                Detect,
            ...
            if m in {Detect, FishDetect, FishDetectAux, FishDetectLSCD, FishDetectLSDECD, YOLOEDetect, Segment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26}:
                m.legacy = legacy
        elif m is v10Detect:
